...
public class PdfGenerateController {
    ...
    @Operation(summary = "Generate pdf of candidate or company")
    @ApiResponses({
    ...
    @GetMapping("/generate")
    public ResponseEntity<byte[]> processGeneratePdf(@RequestParam String gateway) {
        ResponseEntity<byte[]> response = this.generatePdfService.processPdf(gateway);
    ...
    public ResponseEntity<MessageRequest> getHeadEndpoints(@RequestHeader("Content-Length") String contentLength) {
        log.info("Head request for pdf endpoint");
    ...
    public ResponseEntity<?> getOptionsEndpoints() {
        log.info("Options request for pdf endpoint");
