...
import { API_BASE_URL, API_BASE_URL_CERTIFICATE_SOFT_DELETE, API_BASE_URL_CERTIFICATE_SOFT_TRUE_LIST, API_BASE_URL_COMPANY_WISE_COUNT, API_BASE_URL_SALARY, API_EXCEL_URL, API_PDF_URL } from "../env";
...
const CandidateList = () => {
 ...
 const breadcrumbItems = [
  ...
  { name: 'View Candidates', link: '/candidateList', active: true },
];
  ...
  const fetchJobs = async () => {
    try {
      const deleted = false;
      const response = await axios.get(`${API_BASE_URL_CERTIFICATE_SOFT_TRUE_LIST}/${deleted}`);
      if (response.data === 0){
  ...
  const handleDocument = async (certificateId, index,changeStatus) => {
    try {
      ...
      if (response.data===0) {
        ...
        //api call for company wise count
        if (Array.isArray(statusUpdateResponse.data)) {
          statusUpdateResponse.data.forEach(async (item) => {
              console.log(item.companyName);
              console.log(item.certificateId);
              await countDocumentsGeneratedCompaniesWise(item.certificateId, item.companyName);
          });
      } else {
          console.log(statusUpdateResponse.data.companyName);
          console.log(statusUpdateResponse.data.certificateId);
          await countDocumentsGeneratedCompaniesWise(statusUpdateResponse.data.certificateId, statusUpdateResponse.data.companyName);
      }
        fetchJobs();
      }
  ...
  const countDocumentsGeneratedCompaniesWise = async (certificateId, companyName) => {
    try {
        const response = await axios.put(`${API_BASE_URL_COMPANY_WISE_COUNT}/${certificateId}/company/${companyName}`);
        console.log("API response data:", response.data);
    } catch (error) {
        console.error("Error fetching company-wise count:", error.message);
    }
};
  ...
  const handleCandidateUpdate = (certificateId) => {
    alert('Update clicked for ID:', certificateId);
  };
  ...
  const handleCandidateDelete = async (certificateId) => {
    try {
      ...
      //Soft Delete
      const response = await axios.put(`${API_BASE_URL_CERTIFICATE_SOFT_DELETE}/${certificateId}`);
        ...
        <div className="main-content">
                <div className="page-content">
                    <div className="container-fluid">
                        <Breadcrumb title="Candidate List" breadcrumbItems={breadcrumbItems} />
                        <div>
                        <ToastContainer />
        ...
        <div className="card-body">
          ...
          <div className="table-responsive">
            <div>
              ...
              <Link to={"/candidatePendingStatusList"}><button type="button" className="btn btn-outline-info" style={{ marginLeft: '15px', margin: '10px' }}>Batch</button></Link>
            </div>
                ...
                {jobs.length === 0 ? (
                  ...
                  jobs.map((job, index) => (
                    <tr key={job.certificateId}>
                      ...
                      <Link to={`/userProfile/${job.certificateId}`}>
                          <img
